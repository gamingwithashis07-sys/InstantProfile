...
interface Stats {
  totalUsers: number; totalAccounts: number; totalDmSent: number; totalPosts: number
  totalAutoReplies: number; totalDmPending: number; totalDmCampaigns: number
  totalProducts: number; totalOrders: number; totalRevenue: number
}
interface Activity { id: number; user: string; action: string; target: string; detail: string; created_at: string }
...
export default function AdminOverview() {
  ...
  useEffect(() => {
    ...
    ]).then(([s, a]) => { setStats(s); setActivity(a.slice(0, 6)) }).catch(() => {})
  }, [])

  if (!stats) return (
    <div><h2 className="text-2xl font-bold mb-6">Dashboard Overview</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">{Array.from({ length: 5 }).map((_, i) => <CardSkeleton key={i} />)}</div>
    </div>
  )
  ...
  const statCards = [
    ...
    { icon: Camera, label: 'Posts', value: stats.totalPosts, color: '#7cb86c' },
  ...
  return (
    <div>
      <motion.h2 initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="text-2xl font-bold mb-6">Admin Dashboard</motion.h2>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
        {statCards.map((s, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}>
            <ClayCard hover={false} className="text-center">
              ...
              <div className="text-xl md:text-2xl font-black" style={{ color: s.color }}><AnimatedCounter end={s.value} /></div>
