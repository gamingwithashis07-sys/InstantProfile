...
import {
  ...
  BarChart3, Activity, ChevronLeft, ChevronRight, Home, Instagram, MessageSquare, Globe, Shield,
} from 'lucide-react'
...
export function AdminSidebar() {
  ...
  return (
    <motion.aside
      ...
      className={cn(
        'fixed left-0 top-0 bottom-0 z-[500]',
        'hidden md:flex',
    ...
    >
      <div className="flex items-center gap-2 mb-4 px-2">
        {!collapsed && <span className="text-sm font-bold text-[#f4a261]">Admin Panel</span>}
        <button
          onClick={() => setCollapsed(!collapsed)}
          className="ml-auto p-2 rounded-[10px] bg-[#e8d5c4]/50 dark:bg-[#2d1f14]/50 text-[#6b5a4c] dark:text-[#9c8a7a] hover:text-[#f4a261] transition-colors"
        >
      ...
      <div className="mt-auto pt-4 border-t border-white/10">
        <Link href="/">
          ...
          >
            ...
            <span className={cn('text-sm', collapsed && 'hidden')}>Main Site</span>
          </motion.div>
        </Link>
      </div>
    </motion.aside>
